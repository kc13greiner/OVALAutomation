...
ovalParts.definitions.definition.each {
    def it -> {
        ...
        boolean correctStatus = status == 'DRAFT'
        ...
        if (correctStatus && olderThanTwoWeeks) {
            ...
            def newStatusChange = new NodeBuilder().status_change(date: "${to.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME).toString()}", 'INTERIM')
//            println newStatusChange

            dates[0].children().add(dates.size() + 1, newStatusChange)
            statusNode.value = 'INTERIM'
        }
    }
}
